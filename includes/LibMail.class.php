...
/*		HtmlBody( text [, charset] )
 *		set the body (message) of the mail
 *		define the charset if the message contains extended characters (accents)
 *		default to us-ascii
 *		$mail->HtmlBody( "<b>mél</b> en français avec des accents", "iso-8859-1" );
 */
function HtmlBody( $htmlbody, $charset="" )
{
	$this->body = strip_tags($htmlbody);
...
function ValidEmail($address)
{
	if( ereg( ".*<(.+)>", $address, $regs ) ) {
 	...
 	if(ereg( "^[^@  ]+@([a-zA-Z0-9\-]+\.)+([a-zA-Z0-9\-]{2}|net|com|gov|mil|org|edu|int|NET|COM|GOV|MIL|ORG|EDU|INT)\$",$address) )
 		return true;
 	else
 		return false;
}
...
function _build_html()
{
	...
	$this->xheaders["Content-Transfer-Encoding"] = "base64";

	$k=0;
	$ata[$k++] = chunk_split(base64_encode($this->htmlbody));
	$this->fullBody .= implode($sep, $ata);
}
} // class Mail
